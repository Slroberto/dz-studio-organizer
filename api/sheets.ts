...
const mapRowToOrder = (row: any[], sheetRowIndex: number): ServiceOrder => {
    return {
        ...
        status: row[4] || 'Aguardando produto', // Default to 'Aguardando produto' if empty
...
export const getOrders = async (): Promise<ServiceOrder[]> => {
    try {
        const response = await window.gapi.client.sheets.spreadsheets.values.get({
            ...
            range: `${ORDERS_SHEET_NAME}!A2:N`, // Assuming headers are in row 1
        });
...
export const updateOrder = async (order: ServiceOrder): Promise<any> => {
    ...
    try {
        const response = await window.gapi.client.sheets.spreadsheets.values.update({
            spreadsheetId: GOOGLE_SHEETS_ID,
            range: `${ORDERS_SHEET_NAME}!A${order._rowIndex}:N${order._rowIndex}`,
            valueInputOption: 'USER_ENTERED',
            resource: {
                values: [row],
            },
