...
const initGisClient = (callback: (tokenResponse: any) => void) => {
  if (!GOOGLE_CLIENT_ID) {
    console.error("VITE_GOOGLE_CLIENT_ID is not set. Authentication will fail.");
    // FIX: Instead of failing silently, throw a user-friendly error that can be caught and displayed in the UI.
    throw new Error("A configuração da aplicação está incompleta. Contate o administrador.");
  }
