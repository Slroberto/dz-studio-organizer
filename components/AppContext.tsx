...
interface AppContextType {
  ...
  addSearchSource: (source: Omit<SearchSource, 'id'>) => Promise<void>;
  updateSearchSource: (source: SearchSource) => Promise<void>;
  ...
  // New functions for opportunities
  addOpportunity: (opportunityData: Omit<Opportunity, 'id'>) => Promise<void>;
  ...
  // File Management
  uploadFilesForOrder: (orderId: string, files: File[]) => Promise<void>;
  removeFileFromOrder: (orderId: string, fileId: string) => Promise<void>;
}
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (savedViews) setKanbanViews(JSON.parse(savedViews));
      
      const savedSources = localStorage.getItem('dz-search-sources');
      if(savedSources) setSearchSources(JSON.parse(savedSources));

    } catch (error) {
      console.error("Failed to load saved views/sources from localStorage:", error);
    }
  }, []);
  ...
  const filteredOrders = useMemo(() => {
     ...
     if (Object.keys(kanbanFilters).length > 0) {
        tempOrders = tempOrders.filter(o => {
            const { searchTerm, client, responsible, startDate, endDate, priority, customFields } = kanbanFilters;
            ...
            if (customFields) {
                for (const fieldId in customFields) {
                    const filterValue = customFields[fieldId];
                    if (filterValue === '' || filterValue === undefined) continue;

                    const orderValue = o.customFields?.[fieldId];
                    const fieldDef = customFieldDefinitions.find(def => def.id === fieldId);

                    if (orderValue === undefined) return false;

                    if (fieldDef?.type === 'text') {
                        if (typeof orderValue === 'string' && !orderValue.toLowerCase().includes(String(filterValue).toLowerCase())) {
                            return false;
                        }
    ...
    return tempOrders;
  }, [orders, kanbanFilters, isStalledFilterActive, customFieldDefinitions]);
  ...
  const addOpportunity = useCallback(async (opportunityData: Omit<Opportunity, 'id'>) => {
    setIsDataLoading(true);
    await new Promise<void>(resolve => {
        setTimeout(() => {
            ...
            addNotification({ message: `Oportunidade "${newOpportunity.title}" criada.`, type: NotificationColorType.Success });
            setIsDataLoading(false);
            resolve();
        }, 500);
    });
  ...
  const updateOpportunity = useCallback(async (updatedOpportunity: Opportunity) => {
    ...
    await new Promise<void>(resolve => {
        setTimeout(() => {
            ...
            addNotification({ message: `Oportunidade "${updatedOpportunity.title}" atualizada.`, type: NotificationColorType.Success });
            setIsDataLoading(false);
            resolve();
        }, 500);
    });
  ...
  const deleteOpportunity = useCallback(async (opportunityId: string) => {
    ...
    await new Promise<void>(resolve => {
        setTimeout(() => {
            const oppToDelete = opportunities.find(o => o.id === opportunityId);
            if (oppToDelete) {
                setOpportunities(prev => prev.filter(o => o.id !== opportunityId));
                addNotification({ message: `Oportunidade "${oppToDelete.title}" excluída.`, type: NotificationColorType.Warning });
            }
             setIsDataLoading(false);
            resolve();
        }, 500);
  ...
  const dismissSuggestedAction = useCallback((messageId: string) => {
    ...
    const uploadFilesForOrder = useCallback(async (orderId: string, files: File[]) => {
        const orderToUpdate = orders.find(o => o.id === orderId);
        ...
        setOrders(prevOrders => prevOrders.map(o => 
            o.id === orderId 
            ? { ...o, files: [...(o.files || []), ...newAttachments] } 
            : o
        ));
        ...
        for (let i = 0; i < newAttachments.length; i++) {
            ...
            try {
                await uploadFile(orderId, file, (progress) => {
                    setOrders(prevOrders => prevOrders.map(o => {
                        if (o.id === orderId) {
                            return { ...o, files: (o.files || []).map(f => f.id === attachment.id ? { ...f, progress: Math.round(progress * 100) } : f) };
                        }
                        return o;
                    }));
                });
                setOrders(prevOrders => prevOrders.map(o => {
                    if (o.id === orderId) {
                        return { ...o, files: (o.files || []).map(f => f.id === attachment.id ? { ...f, uploadStatus: 'completed', progress: 100 } : f) };
                    }
                    return o;
                }));
            } catch (error) {
                setOrders(prevOrders => prevOrders.map(o => {
                    if (o.id === orderId) {
                        return { ...o, files: (o.files || []).map(f => f.id === attachment.id ? { ...f, uploadStatus: 'failed' } : f) };
                    }
                    return o;
                }));
            }
    ...
    const removeFileFromOrder = useCallback(async (orderId: string, fileId: string) => {
        setOrders(prevOrders => prevOrders.map(o =>
            o.id === orderId ? { ...o, files: (o.files || []).filter(f => f.id !== fileId) } : o
        ));
    ...
    useEffect(() => {
        if(searchSources.length > 0) {
            localStorage.setItem('dz-search-sources', JSON.stringify(searchSources));
        }
    }, [searchSources]);

    const addSearchSource = useCallback(async (source: Omit<SearchSource, 'id'>) => {
        const newSource = { ...source, id: `src-${Date.now()}`};
        setSearchSources(prev => [...prev, newSource]);
        addNotification({ message: `Fonte de busca "${newSource.name}" adicionada.`, type: NotificationColorType.Success });
    }, [addNotification]);
    
    const updateSearchSource = useCallback(async (source: SearchSource) => {
        setSearchSources(prev => prev.map(s => s.id === source.id ? source : s));
        addNotification({ message: `Fonte de busca "${source.name}" atualizada.`, type: NotificationColorType.Success });
    }, [addNotification]);

    const deleteSearchSource = useCallback(async (sourceId: string) => {
        setSearchSources(prev => prev.filter(s => s.id !== sourceId));
        addNotification({ message: `Fonte de busca removida.`, type: NotificationColorType.Warning });
    }, [addNotification]);
